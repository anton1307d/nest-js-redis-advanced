...
export class XFetchCacheService {
    ...
    private async isEarlyRecomputeRequired(key: string): Promise<boolean>
    {
       ...
       const delta = await this.cacheService.store.get<number>(XFetchCacheService.DELTA_KEY_PREFIX + key) ?? 1;
    ...
    public async set<T>(key: string, value: T, ttl: number, computedTimeInSec: number = 1): Promise<T>
    {
        await this.cacheService.set<number>(XFetchCacheService.DELTA_KEY_PREFIX + key, computedTimeInSec, ttl);
